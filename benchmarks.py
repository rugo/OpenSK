  ...
  for _ in tqdm(range(args.runs), file=sys.stdout):
    ...
    try:
      ...
      if ex.code.value == ctap.CtapError.ERR.INVALID_COMMAND:
        ...
        error(f"{message} (internal conditions not met): {ex}.")
      elif ex.code.value == 0xF3:  # VENDOR_HARDWARE_FAILURE
        error(f"{message} (internal hardware error).")
      elif ex.code.value == 0xF4:  # PAYLOAD_TOO_LONG
        error(f"{message} (USB HID payload too long).")
      else:
        error(f"{message} (unexpected error: {ex}, {ex.code.value})")
    except Exception as e:  # pylint: disable=broad-except
      error(str(e))
